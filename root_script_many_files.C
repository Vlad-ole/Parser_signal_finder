...
void graph_LeCroy_binary(unsigned int type_of_file)
{
	vector<double> xv_;
	...
	//loop for files
	for(int number_of_file = 0; number_of_file < 22425;  number_of_file++)
	{
		ostringstream f_oss;
		...
		//fit S1 signal
		TGraph gr0(yv_double.size(), &xv_[0], &yv_double[0]);
		...
		ofstream file_out_det("D://Data_work//Am_results_in_detail.dat", ios::app);
		file_out_det << fitFcn.GetParameter(0) << "\t" << fitFcn.GetParameter(1) << "\t" << fitFcn.GetParameter(2) << "\t" << 
		fitFcn.GetParameter(3) <<  "\t" << integral_max << endl;
	
		ofstream file_out_short("D://Data_work//Am_results_short.dat", ios::app);
